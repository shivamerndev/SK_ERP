...
const ChartArea = ({ layout, monthlyFilter, setMonthlyFilter }) => {
    ...
    useEffect(() => {
        ...
        getBillHistory()
            ...
            .catch((err) => {
                console.error("Failed to fetch dashboard bill history:", err);
                if (isMounted) {
                    setLoading(false);
                }
            });
    ...
    // Helper to get latest month and year from the database bills, defaulting to current if empty
    const { targetMonth, targetMonthYear, targetYear } = useMemo(() => {
        if (bills.length === 0) {
            const now = new Date();
            return {
                targetMonth: now.getMonth(),
                targetMonthYear: now.getFullYear(),
        ...
        return {
            ...
            targetMonthYear: latestDate.getFullYear(),
            targetYear: latestDate.getFullYear()
        };
    }, [bills]);

    const revenueTrendPresets = useMemo(() => {
        // 1. Monthly (current/target year, Jan - Dec)
        const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
        const monthly = months.map(name => ({ name, Revenue: 0 }));
        
        // 2. Weekly (of targetMonth, targetMonthYear)
        const weekly = [
        ...
        bills.forEach(bill => {
            ...
            const amt = bill.totals?.amount || 0;
            
            // Populate Monthly
            if (d.getFullYear() === targetYear) {
                const mIdx = d.getMonth();
                if (mIdx >= 0 && mIdx < 12) {
                    monthly[mIdx].Revenue += amt;
                }
            }

            // Populate Weekly
            if (d.getMonth() === targetMonth && d.getFullYear() === targetMonthYear) {
                const day = d.getDate();
                if (day <= 7) weekly[0].Revenue += amt;
                else if (day <= 14) weekly[1].Revenue += amt;
                else if (day <= 21) weekly[2].Revenue += amt;
                else weekly[3].Revenue += amt;
            }

            // Populate Quarterly
            if (d.getFullYear() === targetYear) {
                const mIdx = d.getMonth();
                if (mIdx >= 0 && mIdx <= 2) quarterly[0].Revenue += amt;
                else if (mIdx >= 3 && mIdx <= 5) quarterly[1].Revenue += amt;
                else if (mIdx >= 6 && mIdx <= 8) quarterly[2].Revenue += amt;
                else quarterly[3].Revenue += amt;
            }
        });

        return { Monthly: monthly, Weekly: weekly, Quarterly: quarterly };
    }, [bills, targetMonth, targetMonthYear, targetYear]);

    const currentRevenueData = useMemo(() => {
        return revenueTrendPresets[monthlyFilter] || revenueTrendPresets.Monthly;
    }, [revenueTrendPresets, monthlyFilter]);

    const currentPeriodRevenue = useMemo(() => {
        return currentRevenueData.reduce((sum, d) => sum + d.Revenue, 0);
    }, [currentRevenueData]);

    const formatCurrentPeriodRevenue = useMemo(() => {
        if (currentPeriodRevenue >= 10000000) return `₹${(currentPeriodRevenue / 10000000).toFixed(2)}Cr`;
        if (currentPeriodRevenue >= 100000) return `₹${(currentPeriodRevenue / 100000).toFixed(2)}L`;
        return currencyFormatter(currentPeriodRevenue);
    ...
    const revenueChange = useMemo(() => {
        ...
        if (monthlyFilter === "Weekly") {
            // Compare targetMonth in targetMonthYear with previousMonth
            let prevMonth = targetMonth - 1;
            let prevYear = targetMonthYear;
            if (prevMonth < 0) {
                prevMonth = 11;
                prevYear -= 1;
            }
            ...
            // Compare targetYear with targetYear - 1
    ...
    }, [bills, monthlyFilter, targetMonth, targetMonthYear, targetYear]);
    ...
    const categorySalesData = useMemo(() => {
        ...
        if (sortedItems.length <= 4) {
            result = sortedItems.map((item, idx) => ({
                ...item,
                color: colors[idx % colors.length]
            }));
        } else {
            const top3 = sortedItems.slice(0, 3);
            const othersValue = sortedItems.slice(3).reduce((sum, item) => sum + item.value, 0);
            result = [
                ...top3.map((item, idx) => ({ ...item, color: colors[idx] })),
                { name: "Others", value: othersValue, color: colors[3] }
            ];
        }
        return result;
    }, [bills]);
    ...
    const formatTotalCategorySales = useMemo(() => {
        if (totalCategorySales >= 10000000) return `₹${(totalCategorySales / 10000000).toFixed(2)}Cr`;
        if (totalCategorySales >= 100000) return `₹${(totalCategorySales / 100000).toFixed(2)}L`;
        return currencyFormatter(totalCategorySales);
    }, [totalCategorySales]);
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                                <span className="text-lg font-extrabold text-slate-900 tracking-tight">
                                    {formatCurrentPeriodRevenue}
                                </span>
                                <span className={`${revenueChange >= 0 ? "text-emerald-500" : "text-rose-500"} text-xs font-bold flex items-center`}>
                                    {revenueChange >= 0 ? (
                    ...
                    <div className="flex-1 w-full h-[220px] relative mt-1">
                        {loading ? (
                            <div className="w-full h-full flex items-center justify-center bg-slate-50/50 rounded-xl animate-pulse">
                                <span className="text-xs font-semibold text-slate-400">Loading Revenue Data...</span>
                            </div>
                        ) : (
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={currentRevenueData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                                    ...
                                    <YAxis
                                        ...
                                        tickLine={false}
                                        axisLine={false}
                                        tickFormatter={(v) => (v === 0 ? "₹0" : v >= 10000000 ? `₹${(v / 10000000).toFixed(1)}Cr` : v >= 100000 ? `₹${(v / 100000).toFixed(1)}L` : `₹${v / 1000}k`)}
                                    />
            ...
            {layout.salesByCategory && (
                <div className="bg-white border border-slate-200/80 rounded-2xl p-5 lg:col-span-3 flex flex-col shadow-sm min-h-[350px]">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center gap-5">
                        {loading ? (
                            <div className="w-full h-full flex items-center justify-center bg-slate-50/50 rounded-xl animate-pulse min-h-[220px]">
                            ...
                            <>
                                ...
                                <div className="relative w-36 h-36 flex-shrink-0">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                        ...
                                        <span className="text-base font-extrabold text-slate-800 mt-1">{formatTotalCategorySales}</span>
                                    </div>
                                </div>

                                {/* Pie Legend Details */}
                                <div className="w-full flex flex-col gap-2 max-h-[120px] overflow-y-auto pr-1">
                                    {categorySalesData.map((item, idx) => (
                                        <div key={idx} className="flex items-center justify-between text-xs font-semibold text-slate-600">
                                            ...
                                            <span className="text-slate-400 shrink-0">
                                                {totalCategorySales > 0 ? ((item.value / totalCategorySales) * 100).toFixed(0) : 0}%{" "}
                        ...
                        {loading ? (
                            ...
                            topCustomers.map((cust, idx) => (
                                <div key={idx} className="flex items-center justify-between py-1 bg-slate-50/20 hover:bg-slate-50 px-2.5 rounded-xl transition-colors duration-150">
                                    ...
                                    <span className="text-xs font-bold text-slate-800 shrink-0">{cust.amount}</span>
                                </div>
                            ))
                        )}
                    </div>
